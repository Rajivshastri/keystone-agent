...
def _page_header(settings: AgentSettings) -> str:
  ...
  <div class="nav">
    ...
    <a href="/upload-0096">Upload 0096</a>
    <a href="/pair">Pair</a>
...
{rows_html}
    ...
    @app.get("/upload-0096", response_class=HTMLResponse)
    def upload_0096_page() -> HTMLResponse:
        return HTMLResponse(_upload_0096_page(None))

    @app.post("/upload-0096", response_class=HTMLResponse)
    async def upload_0096_submit(request: Request) -> HTMLResponse:
        import tempfile
        from pathlib import Path as _Path
        ...
        upload = form.get("xls")
        if upload is None or not hasattr(upload, "read"):
            return HTMLResponse(_upload_0096_page({"ok": False, "message": "No file uploaded"}))

        filename = getattr(upload, "filename", "0096.xls") or "0096.xls"
        if not filename.lower().endswith((".xls", ".xlsx")):
            return HTMLResponse(_upload_0096_page(
                {"ok": False, "message": f"Expected .xls or .xlsx, got {filename}"}
            ))

        data = await upload.read()  # type: ignore[union-attr]

        tmpdir = _Path(tempfile.mkdtemp(prefix="keystone_0096_"))
        ...
        try:
            from ws_uploader import upload_0096 as _upload
            result = _upload(str(tmp_path), progress_cb=_cb)
            payload = {
                "ok": result.ok,
        ...
        return HTMLResponse(_upload_0096_page(payload))
...
def _upload_0096_page(result: dict | None) -> str:
    settings = load_settings()
    result_html = ""
    if result is not None:
        if result.get("ok"):
            badge = '<span style="color:#1a7a4a;font-weight:700">SUCCESS</span>'
            card_style = "border:1px solid #b8e5c5;background:#f1fdf4"
        else:
            badge = '<span style="color:#b03030;font-weight:700">FAILED</span>'
            card_style = "border:1px solid #f5b8b8;background:#fdf1f1"
...
<div class="card" style="{card_style}">
  <div class="card-title">{badge} — {fname} ({size} bytes)</div>
  <p style="font-size:13px"><b>Message:</b> {result.get('message', '')}</p>
  {f'<p style="font-size:12px;color:#555"><b>Detail:</b> {result.get("detail")}</p>' if result.get('detail') else ''}
  {progress_html}
</div>
"""
    return f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><title>Upload 0096 · Keystone</title>
{_PAGE_STYLE}</head><body>
{_page_header(settings)}
{_setup_banner()}

<div class="card">
  <div class="card-title">Upload 0096 Block Deals</div>
  <p class="muted" style="font-size:12px">
    Pick a <code>0096_*.xls</code> file generated by the legacy Flask app
    (or by Keystone itself) and upload it to WealthSpectrum through this
    agent. On Windows, the file is re-saved via Microsoft Excel (COM) before
    upload so the BIFF record set matches what Apache POI expects — this is
    the workaround for the WS NullPointerException rejection of
    xlwt/LibreOffice-generated xls files.
  </p>
  <form method="post" action="/upload-0096" enctype="multipart/form-data">
    <p>
      <label for="xls"><b>0096 file</b> (.xls or .xlsx)</label><br/>
      <input type="file" name="xls" id="xls" accept=".xls,.xlsx" required/>
    </p>
    <p><button type="submit">Upload to WealthSpectrum</button></p>
  </form>
</div>
